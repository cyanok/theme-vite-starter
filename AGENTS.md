...
- 默认提供的 `src/layout.html` 实现 Halo 2.26 运行时页面布局契约，并构建为 `templates/layout.html`；修改该布局时保留 `html(head, content)` 参数顺序与两个插入点。
...
项目 Agent Skill 使用 `pnpm skills:update` 更新，更新后复核技能文件与 `skills-lock.json` 的 diff。
